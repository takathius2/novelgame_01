...
screen main_menu():
    ...
    ## 次のコードは、同じタグを持つ他のメニュースクリーンが表示された時にスクリーンを置換します。
    tag menu
...
screen history():
    ...
    ## データが大きくなりすぎる可能性があるため、このスクリーンを予測しないよ
    ## うにしています。
    predict False

    use game_menu(_("ヒストリー"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0, spacing=gui.history_spacing):
        ...
        for h in _history_list:
            ...
            window:
                ...
                ## 次の文は history_height が None の場合でもレイアウトが正し
                ## くなるようにしています。
                has fixed:
                    yfit True
