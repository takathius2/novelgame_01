...
## このファイルは GUI をカスタマイズする基本的なオプションを記載しています。
...
## 景・スライダーのつまみ等、インターフェイスの様々な場所で使います。
...
## スモールカラー。クイックメニューなどの、明るさを調節する必要のある小さなテ
## キストボタンに使います。
...
## バーの非充足部分（右側）やスライダーの背景部分のカラー。バーやスライダーの
## カラーは直接使われず、 GUI を変更・更新した場合の画像生成に使われます。
define gui.muted_color = '#003d51'
define gui.hover_muted_color = '#005b7a'

## text_color は台詞や選択肢のテキストのカラーです。interface_text_color はヒ
## ストリーやヘルプなどそれ以外のテキストのカラーです。
...
## メインメニューとゲームメニューの背景画像。メインメニューはゲーム起動時に最
## 初に表示されるメニュー、ゲームメニューはゲーム中右クリックで呼び出せるメニュ
...
## ダイアローグ（台詞） ##################################################################
##
## 以下の変数は、一度に表示される台詞とキャラクターの名前を、どのようにスクリ
## ーンに表示するか制御します。
...
## 画面に対する、テキストボックスの垂直方向の位置。 0.0 は上端、0.5 は中央、
## 1.0 は下端になります。
...
## キャラクター名を表示するネームボックスのサイズ。None にすると、自動的に決定
## されます。画像を変えたい場合は gui/textbox.png の画像を入れ替えます（デフォル
## ト画像は透明なので表示されません）。
define gui.namebox_width = None
define gui.namebox_height = None

## ネームボックスのボーダーのサイズ。左、上、右、下の順で指定します。ボックス
## のサイズは、その中に表示されるキャラクター名のサイズから更にボーダー分拡張し
## たサイズになります。
define gui.namebox_borders = Borders(5, 5, 5, 5)

## True に設定すると、ネームボックスの背景画像をスケーリングではなくタイリング
## で表示します。
...
## 場合は、ファイル名に idle_、hover_、selected_、selected_hover_ の接頭辞を付け
## ます。
...
## ボタンのボーダーのサイズ。左、上、右、下の順で指定します。ボタンのサイズ
## は、その中のテキストやオブジェクトのサイズから更にボーダー分拡張したサイズに
## なります。
define gui.button_borders = Borders(6, 6, 6, 6)

## True に設定すると、ボタンの背景画像をスケーリングではなくタイリングで表示し
## ます。
...
## ボタンのフレームに対する、テキストの文字揃え。 0.0 は左揃え、0.5 は中央揃
## え、 1.0 は右揃えになります。
...
## のボタンが用意されています。radio と check は環境設定の各項目のボタン（デフ
## ォルトでは同じ画像）。confirm は確認画面の選択肢、page は セーブ・ロード画面
## のページ切り替え、quick はクイックメニュー、 navigation はゲームメニューのメ
## ニュー切り替えに使うボタンです。
...
## 上記以外にも、接頭辞と接尾辞を適切に組み合わせた変数名を追加すれば、
## 様々なカスタマイズが可能になります。例えば、次の行をアンコメントすると
## navigation（メニュー切り替え）ボタンの横幅を指定することができます。
...
## います。他のボタンと同じように gui/button ディレクトリーにある slot_ の接頭辞
## が付いた背景画像を使います。
...
## True に設定すると、フレームの背景画像をスケーリングではなくタイリングで表示
## します。
...
## バー・スクロールバー・スライダーの各々の太さ（水平バーでは縦幅、垂直バーで
## は横幅）。
...
## NVL モード #####################################################################
##
## NVL（ノベル）スクリーンは、 NVL モード（全画面方式）のキャラクターの台詞を
## 表示するスクリーンです。

## NVL モードに使用する背景のボーダー。画像は gui/nvl.png。
define gui.nvl_borders = Borders(0, 15, 0, 30)

## NVL モードにおける、一度に表示されるエントリー（１台詞）の最大数。この値以
## 上のエントリーを表示しようとすると、一番古いエントリーが取り除かれます。
